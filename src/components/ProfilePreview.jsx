...
export default function ProfilePreview(){
    ...
    if(userState.loading){
        ...
        (<div className="dropdown w-1/12 min-w-[2rem] max-w-[4rem] relative">
            <img
                id="outerImage"
                src={user.image ? `${user.image}` : `./${import.meta.env.VITE_DEFAULT_IMAGE}`}
                className="rounded-full w-4/5 filter dark:invert max-h-[3rem] text-xs"
                alt="profile image"
            />
            <div id="innerDiv" className="dropdown-menu hidden rounded-md backdrop-blur-sm flex-col items-center z-10 justify-center p-4 shadow-md absolute border border-gray-100 w-[20vw] h-[70vh] min-w-[250px] min-h-[450px] bg-gray-400 top-full left-full md:left-0 transform md:-translate-x-1/2 -translate-x-full">
                <div className="h-full px-4 text-sm 2xl:text-lg flex flex-col items-left justify-between">
                    <img
                        src={user.image ? `${user.image}` : `./${import.meta.env.VITE_DEFAULT_IMAGE}`}
                        className="rounded-full self-center w-1/3 m-4 bg-white aspect-square filter dark:invert"
                        alt="Profile Image"
                    />
