...
export default function Appbar(){
    ...
    const selectHome = () => {
        ...
        myPostsRef.current.classList.add("border-transparent");
        myPostsRef.current.classList.remove("border-indigo-600");
    }
    const selectMyPosts = () => {
        myPostsRef.current.classList.remove("border-transparent");
        myPostsRef.current.classList.add("border-indigo-600");
    ...
    useEffect(() => {
        ...
        else if(currentLocation === "/my-posts"){
            selectMyPosts()
        }
    },[])
    ...
    return(
        <div className="flex items-center justify-between top-0 w-full py-2 bg-inherit">
            ...
            <div className="flex flex-row items-center">
                <Link ref={homeRef} to={"/"} onClick={selectHome} className="p-2 px-5 mx-2 border-b-4 border-transparent" >
                    <div className="px-auto w-full items-center flex justify-center">
                        Home
                    </div>
                </Link>
                <Link ref={myPostsRef} to={"/my-posts"} onClick={selectMyPosts} className="p-2 px-5 mx-2 border-b-4 border-transparent" >
                    <div className="px-auto w-full items-center flex justify-center whitespace-nowrap">
                        My Posts
                    </div>
                </Link>
                <ProfilePreview MainButtonRefs={[homeRef, myPostsRef]} />
            </div>
        </div>
    );
}